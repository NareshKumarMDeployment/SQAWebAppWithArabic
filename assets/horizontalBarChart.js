...
function drawHorizontalGroupBarChartChart(config) {
    ...
    d3.select(mainDiv).attr("width", $(mainDiv).width()).attr("height", $(mainDiv).height() * 0.80);
    ...
    g.append("g")
        ...
        .call(d3.axisBottom(x).ticks(10, maxTicks).tickFormat(defaultFormat))
        .append("text")
        .attr("x", width / 2)
    ...
    g.selectAll(".text")  		
        ...
        .enter()
        .append("text")
        .attr("fill", "black").attr("font-size","10px")
        .attr("x", function(d) { iInt = iInt + 2; return dataPoints[iInt]; })
        .attr("y", function(d) {yVal = yVal - 28;  return yVal; })
        .attr("dy", ".71em")
        .text(function(d) {return d.classValue; });  

    var xVal = 0, yVal = 240, iInt = -1;
    ...
    g.selectAll(".text")  		
        ...
        .attr("y", function(d) {yVal = yVal - 28;  return yVal; })
        .attr("dy", ".71em")
        .text(function(d) {return d.classBldg; });  
}
...
//             // barHeight = (height-axisMargin-margin*2)* 0.4/pie.data.length,
//             // barPadding = (height-axisMargin-margin*2)*0.6/pie.data.length, labelWidth = 0;
...
//             //     return Math.max(width + valueMargin, d.value);
//             // });
